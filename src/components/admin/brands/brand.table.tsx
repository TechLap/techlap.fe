...
interface IProps {
  ...
  onEditClick: (brand: IBrand) => void;
  onDeleteClick: (brand: IBrand) => void;
  filters: { name?: string; createdAt?: string | null };
  onFilterChange: (key: string, value: string) => void;
}

const BrandTable = (props: IProps) => {
  const { brandData, onEditClick, onDeleteClick, filters, onFilterChange } = props;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: IBrand) => row.createdAt ? dayjs(row.createdAt).format("DD/MM/YYYY") : "",
    },
    {
      key: "updatedAt",
      header: "Ngày cập nhật",
      render: (row: IBrand) => row.updatedAt ? dayjs(row.updatedAt).format("DD/MM/YYYY") : "",
    },
    {
      key: "actions",
      header: "Thao tác",
      render: (row: IBrand) => (
        <>
          <Access permission={{ name: "Update a brand" }} hideChildren>
            <button
              type="button"
          ...
          <Access permission={{ name: "Delete a brand" }} hideChildren>
            <button
              type="button"
              className="inline-flex items-center gap-x-2 px-1 rounded-lg border border-transparent text-gray-800 hover:text-gray-900 hover:bg-gray-50 focus:outline-hidden focus:text-gray-800 disabled:opacity-50 disabled:pointer-events-none"
  ...
  return (
    <DataTable
      data={brandData ?? []}
      columns={columns}
      rowKey={(row: IBrand, index: number) => row.id ?? `brand-${index}`}
    />
  );
};
