...
import { apiResetPasswordForUser } from "../../../config/api";
import { toast } from "react-toastify";
import CustomToast from "../../../components/common/toast.message";

const ResetPasswordPageForUser = () => {
    ...
    const {
        ...
        handleSubmit,
        formState: { errors }
    } = useForm<FormValues>({ resolver: yupResolver
        (changePasswordSchema) as any });
    ...
    const handleChangePassword: SubmitHandler<FormValues> = async (values) => {
        const response = await apiResetPasswordForUser(
            {token: token as string, newPassword: values.newPassword, reNewPassword: values.reNewPassword}
        );
        if (response.data) {
            toast.success(
                <CustomToast
...
export default ResetPasswordPageForUser;